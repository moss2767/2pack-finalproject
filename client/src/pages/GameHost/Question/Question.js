...
const Question = ({ question }) => {
  ...
  useEffect(() => {
    if (answer) {
      const correctOption = document.querySelector(`.option-${answer.option.replace(/\W+/g, '')}`)
    ...
    <>
      ...
      <Grid container spacing={2}>
        { question.answers.map(answer => (
          <Grid key={answer.option} className={`option-${answer.option.replace(/\W+/g, '')}`} item xs={12} sm={6}>
